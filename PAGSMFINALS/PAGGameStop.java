...
public class PAGGameStop {
    ...
    public void addGame() {
        ...
        System.out.print("Enter Game Name: ");       String name  = sc.nextLine();
        System.out.print("Enter Game Genre: ");      String genre = sc.nextLine();
        System.out.print("Enter Game Year: ");       String year  = sc.nextLine();
        System.out.print("Enter Game Developer: ");  String dev   = sc.nextLine();
        System.out.print("Enter Game Platform: ");   String plat  = sc.nextLine();
    ...
    public void editGame() {
        ...
        System.out.print("New Name (leave blank to keep): ");       String name  = sc.nextLine();
        System.out.print("New Genre (leave blank to keep): ");      String genre = sc.nextLine();
        System.out.print("New Year (leave blank to keep): ");       String year  = sc.nextLine();
        System.out.print("New Developer (leave blank to keep): ");  String dev   = sc.nextLine();
        System.out.print("New Platform (leave blank to keep): ");   String plat  = sc.nextLine();

        if (!name.isEmpty())  gameList.set(index,     name);
        if (!genre.isEmpty()) gameList.set(index + 1, genre);
        if (!year.isEmpty())  gameList.set(index + 2, year);
        if (!dev.isEmpty())   gameList.set(index + 3, dev);
        if (!plat.isEmpty())  gameList.set(index + 4, plat);
    ...
    public void deleteGame() {
        ...
        for (int i = 0; i < 5; i++) gameList.remove(index);
        save();
    ...
    public void sortGames() {
        System.out.println("""
                ...
                """);
        System.out.print("Choice: ");
        if (!sc.hasNextInt()) { System.out.println("PLEASE ENTER A NUMBER."); sc.next(); return; }
        ...
        String sortDesc;
        switch (choice) {
            case 1 -> { temp.sort(Comparator.comparing(a -> a[0].toLowerCase())); sortDesc = "Name A-Z"; }
            case 2 -> { temp.sort((a, b) -> b[0].toLowerCase().compareTo(a[0].toLowerCase())); sortDesc = "Name Z-A"; }
            case 3 -> { temp.sort(Comparator.comparing(a -> a[2])); sortDesc = "Year Ascending"; }
            case 4 -> { temp.sort((a, b) -> b[2].compareTo(a[2])); sortDesc = "Year Descending"; }
            default -> { System.out.println("Invalid choice."); Logger.gamestop("Sort failed — invalid choice: " + choice); return; }
        }

        gameList.clear();
        for (String[] game : temp) gameList.addAll(Arrays.asList(game));
        save();
    ...
    public static void read() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader("gameList.txt"))) {
            ...
            while ((line = br.readLine()) != null)
                if (!line.trim().isEmpty()) gameList.add(line);
        } catch (IOException e) {
    ...
    public static void save() {
        try (BufferedWriter bw = new BufferedWriter(new FileWriter("gameList.txt"))) {
            for (String item : gameList) { bw.write(item); bw.newLine(); }
        } catch (IOException e) {
    ...
    public void displayList() {
        if (gameList.isEmpty()) { System.out.println("\n[!] THE LIST IS CURRENTLY EMPTY."); return; }
        printHeader();
        int count = 1;
        for (int i = 0; i < gameList.size(); i += 5) printRow(count++, i);
        System.out.println("-----------------------------------------------------------------------------------------------------------------------");
    }
